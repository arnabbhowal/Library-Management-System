    ...
    while(another=='y')  //infinite loop
    {
        ...
        if(findbook=='t' )
        {
            ...
            if(getch()=='y')
            {
                ...
                fclose(fp);
                remove("File.dat");
                rename("test.dat","File.dat"); //copy all item from temporary file to fp except that
                fp=fopen("File.dat","rb+");    //we want to delete
                if(findbook=='t')
                {
                    gotoxy(10,10);
...
void searchbooks()
{
    ...
    gotoxy( 15,20);
    printf("Enter Your Choice");
    fp=fopen("File.dat","rb+"); //open file for reading propose
